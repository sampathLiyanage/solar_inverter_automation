...
const getNewOutputPrioritySettingBasedOnChargeDischargeData = (batteryVoltage, chargeSum, dischargeSum, timestamp, currentBatterySetting, threshold1 = 500, threshold2 = 250,  multiplier = 1) => {
    const currentTime = timestamp.substring(11,24);
    if (batteryVoltage < '50.7' && currentBatterySetting === 'SBU') {
        return SUB_VAL;
    } else if (currentTime > '15:00:00' && currentTime < '19:00:00' && ((+chargeSum) - (+dischargeSum)) * 0.1 * multiplier < threshold1 && currentBatterySetting === 'SBU') {
        return SUB_VAL;
    } else if (currentTime > '15:00:00' && currentTime < '19:00:00' && ((+chargeSum) - (+dischargeSum)) * 0.1 * multiplier > threshold1 && currentBatterySetting === 'SUB') {
        return SBU_VAL;
    } else if (currentTime > '19:00:00' && currentTime < '22:00:00' && ((+chargeSum) - (+dischargeSum)) * 0.1 * multiplier < threshold2 && currentBatterySetting === 'SBU') {
        return SUB_VAL;
    } else if (currentTime > '19:00:00' && currentTime < '22:00:00' && ((+chargeSum) - (+dischargeSum)) * 0.1 * multiplier > threshold2 && currentBatterySetting === 'SUB') {
        return SBU_VAL;
    } else if ((currentTime > '22:00:00' || currentTime < '15:00:00') && currentBatterySetting === 'SUB' && batteryVoltage > '51.3') {
        return SBU_VAL;
    }
    return false;
}
