...
const getNewOutputPrioritySettingBasedOnChargeDischargeData = (batteryVoltage, chargeSum, dischargeSum, timestamp, currentBatterySetting, threshold1 = 0.5, threshold2 = 0.25) => {
    const currentTime = timestamp.substring(11,24);
    if (batteryVoltage < '50.7' && currentBatterySetting === 'SBU') {
        return SUB_VAL;
    } else if (currentTime > '15:00:00' && currentTime < '19:00:00' && (((+chargeSum) - (+dischargeSum))/(+chargeSum)) < threshold1 && currentBatterySetting === 'SBU') {
        return SUB_VAL;
    } else if (currentTime > '15:00:00' && currentTime < '19:00:00' && (((+chargeSum) - (+dischargeSum))/(+chargeSum)) > threshold1 && currentBatterySetting === 'SUB') {
        return SBU_VAL;
    } else if (currentTime > '19:00:00' && currentTime < '22:00:00' && (((+chargeSum) - (+dischargeSum))/(+chargeSum)) < threshold2 && currentBatterySetting === 'SBU') {
        return SUB_VAL;
    } else if (currentTime > '19:00:00' && currentTime < '22:00:00' && (((+chargeSum) - (+dischargeSum))/(+chargeSum)) > threshold2 && currentBatterySetting === 'SUB') {
        return SBU_VAL;
