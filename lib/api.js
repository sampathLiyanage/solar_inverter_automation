...
const getChartDataUptoNowForToday = async (authData, chart, fromTime) => {
  ...
  let month = ("0" + (date_lk.getMonth() + 1)).slice(-2);
  let date = ("0" + date_lk.getDate()).slice(-2);
  let hours = ("0" + date_lk.getHours()).slice(-2);
  let minutes = ("0" + date_lk.getMinutes()).slice(-2);
  let seconds = ("0" + date_lk.getSeconds()).slice(-2);
  ...
  const chartUrl = REMOTE_URL + '?' + new URLSearchParams(params).toString();
